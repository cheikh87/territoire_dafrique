...
$('#new_s_').on('click', function (e) {
  ...
  $(this).summernote('destroy');
   ...
   $('.description').summernote({
    ...
    $('.title_activity').text('Ajout d\'une nouvelle publication');
    $('#edit_').hide();
    $('#add_').show(); 
    $('#new_s').modal('show');  
...
$('#table_activites tbody').on('click', '.modifier', function (e) {
    ...
    $('.title_activity').text('Modifier une maladie');
...
$('#table_activites tbody').on('click', '.supprimer', function (e) {
    e.preventDefault();
    var data = table.row($(this).parents('tr')).data();
    $('.title2').html('<span style="color:blue">Suppression de </span> '+data.titre);
    $('.confirm_action').html("supprimer <span style='color:red'>"+data.titre);
